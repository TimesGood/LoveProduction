 ...
 *    当页卡量不多时，使用FragmentPagerAdapter
 *    当页卡量比较多时，使用FragmentStatePagerAdapter
 *    区别：前者每生成一个Fragment都会储存在内存中，后者则只保留当前显示的Fragment，
 *    离开视线的就会被销毁，等需要显示时再创建
 *    即前者速度快但占内存，量多时OOM。后者速度慢，但始终只有一个实例，内存消耗少
 */
