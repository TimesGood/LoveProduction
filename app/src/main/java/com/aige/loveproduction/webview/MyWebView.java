package com.aige.loveproduction.webview;
...
//		webSetting.setDatabasePath(mContext.getApplicationContext().getDir("databases", 0).getPath());
        ...
        /*=================================滚动条设置===================================*/
		getX5WebViewExtension().setHorizontalScrollBarEnabled(false);//设置水平方向滚动条不可用
		getX5WebViewExtension().setVerticalScrollBarEnabled(false);//设置竖直方向滚动条不可用
		
		//使用WebChormClient的特性处理html页面
		webChromeClient = new MyWebChromeClient(getContext(),this);
		this.setWebChromeClient(webChromeClient);
		//使用WebViewClient的特性处理html页面
		webViewClient = new MyWebViewClient(getContext(),this);
